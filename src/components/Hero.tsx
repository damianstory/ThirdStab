...
const text = {
  en: {
    ...
    subtitle2: {
      bold: 'The January activity is now live! Scroll down ',
  ...
  fr: {
    ...
    subtitle2: {
      bold: "L'activité de janvier est maintenant en ligne ! Faites défiler la page ",
    ...
    badge: 'Activité de janvier en ligne'
  }
};
...
export default function Hero({ language = 'en' }: HeroProps) {
  ...
  const handleScrollToJanuary = () => {
    const januaryCard = document.getElementById('january-activity-card');
    if (januaryCard) {
      const headerHeight = 64; // Height of fixed header
      const extraOffset = 250; // Extra offset to show cards below
      const elementPosition = januaryCard.getBoundingClientRect().top;
